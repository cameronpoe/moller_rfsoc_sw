...
static int parse_positive_size(
	const char *text,
	size_t *value) {
	...
	*n_windows = 0;
	...
	for (size_t i = 0; i < max_pairs; i++) {
		...
		if (valid > 0 && rising[i] < result[valid - 1].end_ts) {
			fprintf(stderr,
				"Gate windows overlap or are not sorted at "
				"%zu\n",
				i);
			free(rising);
			free(falling);
...
static int accumulate_block(
	...
	size_t *window_index) {
	if (!ch2 || !ch3 || !windows || !int_ch2 || !int_ch3 || !window_index) {
		return -1;
	}
...
int main(
	...
	if (parse_bool(argv[6], &fft_enabled) != 0) {
		fprintf(stderr,
			"Invalid FFT_ON: %s (expected true, True, false, or "
			"False)\n",
			argv[6]);
		return EXIT_FAILURE;
	}
	...
	if (status != 0 || n_windows < 2) {
		fprintf(stderr,
			"At least two valid gate windows are required.\n");
		goto cleanup;
	}
	...
	const size_t block_words = DMA_PACKETS_PER_BLOCK * words_per_packet;
	const size_t samples_per_packet = (words_per_packet - 2) / 3;
	...
	if (block_words > SIZE_MAX / sizeof(*raw_block) ||
	...
	raw_block = malloc(block_words * sizeof(*raw_block));
	...
	if (!raw_block || !samples || !dc_ch2 || !dc_ch3) {
	...
	for (;;) {
		...
		size_t words_read =
			fread(raw_block, sizeof(*raw_block), block_words, adc);
		clock_gettime(CLOCK_MONOTONIC, &end);
		read_seconds += elapsed_seconds(&start, &end);

		if (words_read == 0) {
			if (ferror(adc)) {
				fprintf(stderr,
					"Error while reading ADC file: %s\n",
					strerror(errno));
				goto cleanup;
			}
			break;
		...
		if (complete_packets == 0) {
			fprintf(stderr,
				"ADC file ends with no complete DMA packet.\n");
			goto cleanup;
		}
		if (complete_words != words_read) {
			fprintf(stderr,
				"Ignoring %zu trailing ADC words after the "
				"last "
				"complete DMA packet.\n",
		...
		if (status != 0) {
			fprintf(stderr,
				"adc_packet_parser_v2 failed in block %zu: "
				"%d\n",
				block_number,
				status);
			goto cleanup;
		...
		clock_gettime(CLOCK_MONOTONIC, &start);
		int thread_error =
			pthread_create(&thread2, NULL, dc_worker, &job2);
		if (thread_error == 0) {
		...
		if (!thread2_started || !thread3_started || join2_error != 0 ||
			join3_error != 0) {
			goto cleanup;
		}
		if (job2.status != 0 || job3.status != 0) {
			fprintf(stderr,
				"process_to_dc failed in block %zu: ch2=%d, "
				"ch3=%d\n",
				block_number,
				job2.status,
				job3.status);
		...
		if (job2.n_out != job3.n_out) {
			fprintf(stderr,
				"DC output mismatch in block %zu: ch2=%zu, "
				"ch3=%zu\n",
				block_number,
				job2.n_out,
				job3.n_out);
		...
		if (status != 0) {
			fprintf(stderr,
				"Integration failed in block %zu: %d\n",
				block_number,
				status);
			goto cleanup;
		}

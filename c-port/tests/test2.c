...
int dump_process_to_dc_csv(const char *raw_path, const char *out_csv, int channel, size_t fft_len, size_t max_print) {

    if (!raw_path || !out_csv) return -1;
    ...
    size_t n_read = fread(raw_words, sizeof(uint64_t), WORDS_PER_PACKET, fp);
    fclose(fp);

    if (n_read != WORDS_PER_PACKET) {
        fprintf(stderr, "Expected %llu words, read %zu\n",
                (unsigned long long)WORDS_PER_PACKET, n_read);
        free(raw_words);
        return -4;
    }

...
def plot_resolution(rdf1, rdf2, ch1_name, ch2_name, dir_path):
    ...
    axs[0,0].text(0.08*np.max(n), 0.85*bins[-1], f'$\\sigma={round(np.std(rdf1_ppm), sigfigs=3)}$ ppm', fontdict=dict(size=14))
    ...
    axs[1,0].text(0.25*bin_centers[-1], 0.15*np.max(n_ddf*1e6), f'$\\chi^2/dof={round(chi2/dof, sigfigs=3)}$', fontdict=dict(size=14))
